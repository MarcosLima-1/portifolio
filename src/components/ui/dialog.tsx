...
export function DialogProvider({
	...
	const handleChangeDialogState = useCallback(
		...
		[delayCloseTime, isDialogOpen, isTransitioning, onDialogClose, onOpenChange],
	);
 ...
 */
export function DialogTrigger({ children, disabled, asChild, onClick, className }: DialogTriggerProps) {
	...
	return (
		<button className={className} type="button" disabled={asChild ? false : disabled} onClick={handleClick}>
			{children}
		</button>
	);
}
...
export function DialogCloseButton({ className, children, asChild, ...props }: ButtonProps) {
	...
	return (
		<Button className={cn("absolute top-2 right-2 z-50", className)} variant="ghost" onClick={handleClick} {...props}>
			<XIcon />
		</Button>
	);
}
